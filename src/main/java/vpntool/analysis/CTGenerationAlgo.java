...
//
//            ArrayList<HashMap<String, String>> allBindingMap = new ArrayList<>();
//
////            Iterator<String> varIterator = varSet.iterator();
////            while (varIterator.hasNext()) {//迭代变量
////                String var = varIterator.next();
////
////                for (Place prePlace : bindingOfAllPlaces.keySet()) {
////                    ArrayList<HashMap<String, String>> bindingOfOnePlace = bindingOfAllPlaces.get(prePlace);
////                    for (int j = 0; j < bindingOfOnePlace.size(); j++) {
////                        HashMap<String, String> bindingOfOneToken = bindingOfOnePlace.get(j);
////                        for (String varInBinding : bindingOfOneToken.keySet()) {
////                            if (var.equals(varInBinding)) {
////
////                            }
////                        }
////                    }
...
////            }
//
//            HashMap<String, String> bindingAfterIntegration = new HashMap<>();//这里集成的是所有前集库所
//            Iterator<Place> placeIter = bindingOfAllPlaces.keySet().iterator();
//            while (placeIter.hasNext()) {
//                Place prePlace = placeIter.next();
//                ArrayList<HashMap<String, String>> possibleBindingOfOnePlace = bindingOfAllPlaces.get(prePlace);
//                for (int j = 0; j < possibleBindingOfOnePlace.size(); j++) {
//                    HashMap<String, String> bindingOfOnePlace = possibleBindingOfOnePlace.get(j);
//                    if (placeIter.next() == null) {//说明到最后一个前集库所
//
//                    } else {
//
//                    }
...
//        }
        ...
        for (int i = 0; i < transitions.size(); i++) {
//            boolean isEnabled = true;//标记该变迁是否使能。需要吗？不需要。因为只要找到使某一变迁使能的绑定就直接返回了，并不用记录某一变迁是否使能
...
//                    HashMap<String, ArrayList<String>> constraintFunction = node.getParentNode().getConstraintFunction();//父节点的γ函数
                    ...
                    if (!isInstantiated) {
                        ...
                        System.out.println("其双亲节点为" + node.getParentNode().getUserObject());//这句话有问题
            ...
            HashSet<String> varSet = calcVarSetToMapInBinding(transition);//得到需要映射的变量集，注意这其中的变量顺序并不确定
//            HashMap<String, String> allBindingMap = generateBinding(varSet, petriNet.getConstants());//

            /*————debug————*/
            System.out.println("变迁" + (transition.getId() + 1) + "结束变量集大小的计算，大小为" + varSet.size());
            /*————debug————*/

            /*————debug————*/
            Iterator<String> varInVarSet = varSet.iterator();
            System.out.print("变量集为");
            while (varInVarSet.hasNext()) {
                System.out.print(varInVarSet.next() + ",");
            }
            System.out.println();
            /*————debug————*/

//            ArrayList<ArrayList<String>> allPermutation = Permutation.generateArrayListForEqualNum(petriNet.getConstants(), varSet.size());//生成所有指定数量常量的排列。现在用的是所有常量太多了，会有空间爆炸，考虑先缩小到前集库所中出现的常量。等下，那实化的常量？

            /*根据当前结点的γ，把用来实化的绑定直接存进去。退一步，不存绑定，存用来实化的绑定的常量？*/
            ArrayList<String> relatedConstants = calcConListToMapInBinding(transition, node);
            /*————debug————*/
            System.out.println("变迁" + (transition.getId() + 1) + "结束常量集大小的计算，大小为" + relatedConstants.size());
            /*————debug————*/

            ArrayList<ArrayList<String>> allPermutation = Permutation.generateArrayListForEqualNum(relatedConstants, varSet.size());

            /*————debug————*/
            System.out.println("排列数为" + allPermutation.size());
            /*————debug————*/

            Permutation.freshGlobalVar();//有必要吗？

            /*————debug————*/
            System.out.println("变迁" + (transition.getId() + 1) + "结束所有排列的生成");
            /*————debug————*/

            /*————debug————*/
            // TODO: 修改可能绑定的生成函数，按照token形式
            HashMap<String, HashSet<String>> mappingConstraint = constraintMapping(transition, node);
            /*————debug————*/

            /*————debug————*/
            for (String var : mappingConstraint.keySet()) {
                System.out.print("变量" + var + "可以映射成");
                HashSet<String> cons = mappingConstraint.get(var);
                Iterator<String> conIter = cons.iterator();
                while (conIter.hasNext()) {
                    String con = conIter.next();
            ...
            for (int j = 0; j < allPermutation.size(); j++) {
                ...
                /*————debug————*/
                System.out.println("变迁" + (transition.getId() + 1) + "的判断进入排列的循环");
                /*————debug————*/


                HashMap<String, String> bindingMap = generateBinding(varSet, allPermutation.get(j));
//                boolean isEnabledUnderTheBinding = true;//标记该变迁是否使能
...
//                /*————debug————*/
                ...
                if (bindingMap.size() == 0) {
                    System.out.println("没有生成正常的绑定");
                }
                /*————debug————*/

                /*————debug————*/
                ArrayList<String> consToTest = allPermutation.get(j);
                if (consToTest.get(0).equals("lockNumber")&&consToTest.get(1).equals("lockLockerInfo")&&consToTest.get(2).equals("ee")&&consToTest.get(3).equals("client")&&consToTest.get(4).equals("data")&&consToTest.get(5).equals("gymLockerPT")) {
                    System.out.println("排列中正确的常量集");
                }
                /*————debug————*/
                ...
                if (!bindingSatisfyConstraint(bindingMap, mappingConstraint)) {
                    ...
                    /*————debug————*/
                    System.out.println("变迁" + (transition.getId() + 1) + "的绑定不满足约束");
                    /*————debug————*/
...
//                ArrayList<Arc> inputArcs = transition.getInputArcs();
//                for (int k = 0; k < inputArcs.size(); k++) {
//                    Arc inputArc = inputArcs.get(k);
//
//                    // TODO: 现在只能处理前集实库所的情况
//                    Place prePlace = (Place) inputArc.getSource();//按照初始输入弧的情况获取source
//                    if (prePlace instanceof RealPlace) {
//                        /*等下，能不能在输入token的时候就整合呢？目前先在这里处理吧*/
//                        HashMap<ArrayList<String>, Integer> tokensAfterIntegration = integrateTokens((RealPlace) prePlace);//将库所中token按类型整合在一起
//
//                        ArrayList<String[]> tuplesInArcLabelAL = inputArc.getTuplesInArcLabel();//可省略
//                        HashMap<ArrayList<String>, Integer> tuplesAfterMapping = formalToActualInTuple(tuplesInArcLabelAL, bindingMap);//弧标签经映射后得到的元组
//
//                        if (!tokensSatisfyTuples(tokensAfterIntegration, tuplesAfterMapping)) {
//                            isEnabledUnderTheBinding = false;
//                            break;//只要有一个前集库所不满足则变迁不可发生
//                        }
//
//                    }
//                }
