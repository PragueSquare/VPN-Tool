...
//            HashMap<Place, ArrayList<HashMap<String, String>>> bindingOfAllPlaces = new HashMap<>();//所有前集库所维护一个哈希表
        ...
        if (node.getUserObject().equals("C6") && transitionId == 6) {
            System.out.println("C6和t7的前集库所数量为" + inputArcs.size());
        }
        /*————debug————*/
...
//                HashMap<String, HashMap<Token, Integer>> MAfterIntegration = integrateTokens(curMarking);
    ...
    public ArrayList<CTNode> enableTransition(int transitionId, CTNode node) {
        ...
        /*————debug————*/
        if (node.getUserObject().equals("C20")) {
            System.out.println("针对C20考察变迁" + (transitionId + 1));
        }
        /*————debug————*/

        /*————debug————*/
        if (node.getUserObject().equals("C6") && transitionId == 6) {
            System.out.println("针对C6考察变迁" + (transitionId + 1));
        }
        /*————debug————*/
...
//        }
        ...
        /*————debug————*/
        if (node.getUserObject().equals("C20") && transitionId == 2) {
            System.out.println("C20处理前的绑定数为" + bindingCombination.size());
        }
        /*————debug————*/
        ...
        /*————debug————*/
        if (node.getUserObject().equals("C20")) {
            System.out.println("C20处理后的绑定数为" + allBindingMap.size());
        }
        /*————debug————*/

        /*————debug————*/
        if (node.getUserObject().equals("C11") && transitionId == 2) {
            System.out.println("C11和t3处理后的绑定数为" + allBindingMap.size());
        }
        /*————debug————*/
                                ...
                                if (linkFunction.get(k)[0].equals("true")) {// TODO: 布尔表达式的解析
                                    ...
                                    if (oper.equals("+")) {
                                        if (!constraintFunction.containsKey(var)) {//γ中还没有关于此变量的映射
                                            ...
                                            // TODO: 防止重复。但是发生重复的物理意义合适吗？
                                            if (!cons.contains(newPlace)) {
                                                cons.add(newPlace);
                                            }
...
//                }
    ...
    //实为生成所有子节点（后续状态）
//    public ArrayList<CTNode> enableTransition2(int transitionId, CTNode node) {
//        HashMap<String, ArrayList<String>> origConstraintFunction = node.getConstraintFunction();//本结点的γ函数
//
//        /*————debug————*/
//        System.out.println("enableTransition被调用");
...
//            }
    ...
    public CTNode findDuplicatedNode(CTNode node, ArrayList<CTNode> reachabilitySet) {
